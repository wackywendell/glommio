...
//! ## Attention
//!
//! This crate was previously named Scipio but was renamed after a trademark
//! dispute. We are removing this message soon but it is now here to avoid
//! confusion.
//!
//! ## What is Glommio
//!
//! Glommio is a library providing a safe Rust interface for asynchronous I/O,
//! based on the linux `io_uring` interface and Rust's `async` support.
//! Additionally, Glommio provides support for pinning threads to CPUs, allowing
...
//! as necessary. For a thread-per-core system, each thread must handle its own
//! scheduling at the application level.
//!
...
//! post](https://www.scylladb.com/2018/06/12/scylla-leverages-control-theory/)
//! from Glauber.
//!
//! ## Prior work
//!
...
//!
//! Take as an example the [async-io](https://github.com/stjepang/async-io)
//! crate. It has `park()` and `unpark()` methods. One can `park()` the current
//! executor, and a helper thread will unpark it. This allows one to effectively
//! use that crate with very little need for anything else for the simpler
//! cases. Combined with synchronization primitives like `Condvar`, and other
//! thread-pool based future crates, it excels in conjunction with others but it
...
//!
//! Please note Glommio requires at least 256 KiB of locked memory for
...
//!
//! * There is not yet any cross-shard communication nor ergonomic primitives to
//!   allow for cross-shard services. This allows one to implement simple,
//!   independent sharded systems but would need to happen before more complex
//!   work can be built on top of this crate.
//!
...
//!
//! * The networking code uses `poll + rw`. This is essentially so we could get
//!   started sooner by reusing code from
//!   [async-io](https://github.com/stjepang/async-io) but we really should be
